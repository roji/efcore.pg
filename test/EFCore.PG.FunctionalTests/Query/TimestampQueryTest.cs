...
WHERE e.""TimestamptzDateTime"" = @__dateTime_0");
        ...
        [ConditionalTheory]
        [MemberData(nameof(IsAsyncData))]
        public async Task Compare_timestamptz_column_to_timestamp_column_with_ToUniversalTime(bool async)
        {
            using var ctx = CreateContext();

            await AssertCount(
                async,
                ss => ss.Set<Entity>().Where(e => e.TimestamptzDateTime == e.TimestampDateTime.ToUniversalTime()));
...
WHERE e.""TimestamptzDateTime"" = e.""TimestampDateTime""::timestamptz");
        ...
        [ConditionalTheory]
        [MemberData(nameof(IsAsyncData))]
        public async Task Compare_timestamptz_column_to_timestamp_column_with_ToLocalTime(bool async)
        {
            using var ctx = CreateContext();

            await AssertCount(
                async,
                ss => ss.Set<Entity>().Where(e => e.TimestamptzDateTime.ToLocalTime() == e.TimestampDateTime));
