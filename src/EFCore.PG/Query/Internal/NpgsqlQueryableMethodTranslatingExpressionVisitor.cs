        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            if (method.DeclaringType == typeof(NpgsqlQueryableExtensions))
            {
                ...
                if (source is ShapedQueryExpression shapedQueryExpression)
                {
                    ...
                    {
                        nameof(NpgsqlQueryableExtensions.StringAggregate)
                            when genericMethod == NpgsqlQueryableExtensions.StringAggregateWithoutSelectorMethod
                            => CheckTranslated(
                                TranslateStringAggregate(
                                    shapedQueryExpression.UpdateResultCardinality(ResultCardinality.Single),
                                    methodCallExpression.Arguments[1], selector: null)),
